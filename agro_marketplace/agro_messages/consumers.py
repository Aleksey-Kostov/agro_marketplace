...
class MessageConsumer(AsyncJsonWebsocketConsumer):
    """
    WebSocket consumer for a single message conversation.

    The conversation is identified by an existing Message instance.
    The server derives the conversation participants and product context
    from that message instead of trusting the client.
    """

    async def connect(self):
        self.user = self.scope["user"]

        self.root_message_id = self.scope["url_route"]["kwargs"].get(
            "message_id"
        )
